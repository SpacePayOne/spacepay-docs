...
<Note>
  **Withdrawals** must be enabled for your merchant account. If withdrawals are disabled, contact
  the SpacePay support team.
</Note>
...
<ParamField body="amountInCents" type="number" required>
  Withdrawal amount in cents (for example, 5000 = $50.00). Minimum: **100** cents, maximum:
  **100000000** cents.
</ParamField>

<ParamField body="chainId" type="number" required>
  EVM chain ID to settle the withdrawal on. Use your withdrawal config endpoint to get supported
  chain IDs.
</ParamField>
...
<ResponseField name="status" type="string" required>
  Initial status will be `pending`. Possible values: `pending`, `processing`, `completed`, `failed`,
  `cancelled`.
</ResponseField>
