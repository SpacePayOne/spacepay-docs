...
<ResponseField name="status" type="string" required>
  Current withdrawal status. Possible values: - `pending`: Withdrawal created, waiting to be
  processed - `processing`: Withdrawal is being submitted on-chain - `completed`: Withdrawal
  confirmed and settled - `failed`: Withdrawal failed (see `errorMessage`) - `cancelled`: Withdrawal
  cancelled
</ResponseField>
