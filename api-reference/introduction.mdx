...
<Warning>
  ...
  and server-side implementations only.
</Warning>
...
<Note>
  **Webhook Secret**: Each webhook endpoint gets its own unique secret key for signature
  verification. Set up webhooks in your admin dashboard under Settings → Developers → Webhooks.
</Note>
