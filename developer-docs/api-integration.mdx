...
**Merchant secret key** (`X-SpacePay-Secret-Key`): use on your server for [creating payments or deposits](/api-reference/endpoint/external-payments-create), [reading a payment by id](/api-reference/endpoint/external-payments-get), and [withdrawals](/api-reference/endpoint/external-withdrawals-create).
...
[`GET /v1/external/secretkey-auth/payments`](/api-reference/endpoint/external-payments-list) supports optional query params: `type`, `status`, `search`, `sortBy`, `sortOrder`, `limit`, and `offset`.
