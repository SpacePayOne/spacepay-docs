...
<Note>
  ...
  quotes, deposit address, or receipt — call the Get Payment or Get Withdrawal endpoints in
  **API reference → Payments & Deposits / Withdrawals** using the `id` from the payload.
</Note>
...
app.post('/webhooks/spacepay', (req, res) => {
  ...
  switch (event.type) {
    case 'payment.created':
      ...
      console.log(`Amount: ${event.data.payment.amountInCents} ${event.data.payment.currency}`);
      console.log(`Deposit Address: ${event.data.payment.depositAddress.address}`);
      break;
